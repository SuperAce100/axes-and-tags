...
class P5JSSketchGen(Domain):
    def __init__(self, concept: str, data_dir: str, model: str = text_model, console: Console = Console()):
        super().__init__(name="p5js", display_name=concept, data_dir=data_dir, model=model, console=console)
    ...
    def generate_multiple(self, n: int, examples: str, old_tags: List[str]) -> List[str]:
        return generate_p5js_multiple(self.concept, examples, n, old_tags, text_model=self.model)
    ...
    def feedback_examples(self, feedback: Dict[str, List[str]], results_dir: str) -> str:
        return load_p5js_from_feedback(self.concept, feedback, results_dir)
    ...
    def save_result(self, results: List[Tuple[str, str]], path: str = None) -> str:
        ...
        p5js_results = [r for r in results[1]]
        tags = [r for r in results[2]]

        save_p5js(p5js_results, prompts, tags, path)
        return path
