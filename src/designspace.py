...
from models.prompts import fill_design_space_prompt, create_design_space_prompt, explore_axis_prompt
...
class DesignSpace(BaseModel):
    ...
    def __str__(self):
        return "\n".join([f"{axis.name} ({axis.status}) : {axis.value}" for axis in self.axes])
    
    @staticmethod
    def create(concept: str, domain: str, model: str = text_model):
        response = llm_call(create_design_space_prompt.format(concept=concept, domain=domain), model=model)
    ...
    def fill(self, model: str = text_model):
        ...
        """
        unconstrained_axes = [axis for axis in self.axes if axis.status == "unconstrained" and axis.value == ""]

        if not unconstrained_axes:
            return

        unconstrained_axes_str = "\n".join([f"{axis.name}: {axis.value}" for axis in unconstrained_axes])

        response = llm_call(fill_design_space_prompt.format(concept=self.concept, domain=self.domain, axes=unconstrained_axes_str), model=model)
